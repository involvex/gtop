...
var footerBox = blessed.box({
  ...
  width: '100%',
  height: 1,
  content: '{bold}Hotkeys:{/bold} [k]KILL [r]Cache [C]Standby [q]Quit',
  style: {
    fg: 'white',
    bg: 'blue',
  },
...
function killSelectedProcess() {
  ...
  var data = procTable.table.getData();
  if (data && data.data && data.data[selected]) {
    var pid = data.data[selected][0];
        ...
        fkill(pid, { force: true, tree: true })
          .then(function () {
            footerBox.setContent(
              '{bold}Hotkeys:{/bold} [k]KILL [r]ClearCache [C]ClearStandby [q]Quit {green}Killed PID ' +
                pid +
                '{/green}'
            );
            screen.render();
          ...
          .catch(function (_err) {
            ...
            try {
              ...
              footerBox.setContent(
                '{bold}Hotkeys:{/bold} [k]KILL [r]ClearCache [C]ClearStandby [q]Quit {red}Failed to kill PID ' +
                  pid +
                  '{/red}'
              );
            }
...
function resetFooter() {
  footerBox.setContent(
    '{bold}Hotkeys:{/bold} [k]KILL [r]ClearCache [C]ClearStandby [q]Quit'
  );
  screen.render();
}
